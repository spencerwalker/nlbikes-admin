...
function FilesService( $q, $http, apiurl, $cookieStore, appname ) {
    var service = {
        Upload: _upload
    };
    ...
    function _upload(file, fileName) {
        ...
        $http.post(fileURL + '?filename=' + fileName, fd, { transformRequest: angular.identity, headers: {'Content-Type': undefined, 'Authorization': 'Bearer ' + $cookieStore.get(appname + '.token')}})
